...
import CareerDetailPopup from "./CareerDetailPopup";
...
const CareerRoadmapView: React.FC<CareerRoadmapViewProps> = ({
    ...
    const [popupData, setPopupData] = useState<CareerDetailData | null>(null);

    const handleChildClick = async (parent: string, child: string) => {
        const payload = {
            user_history: state.sequenceTokens ?? [],         // secondPayload
            user_recommendation: [parent, child]              // 현재 클릭된 쌍
        };
        console.log("feat3 request ▶︎", payload);
        ...
        try {
            const res = await fetch("http://localhost:5000/feat3/practice", {
                method: "POST",
            ...
            console.log("feat3 response ▶︎", data);           // 결과 로그
            setPopupData(data);
        } catch (err) {
            console.error("feat3 error:", err);
        } finally {
            setIsLoading(false);
        }
    };
                                    ...
                                    <div className="space-y-6">
                                        {tree.children.map((child, childIndex) => (
                                            <div key={childIndex} className="relative">
                                                ...
                                                <div className="pl-6">
                                                    <div className="cursor-pointer bg-gray-800/60 border border-gray-600/40 rounded-lg p-4 hover:bg-gray-800/80 transition-all duration-200 hover:scale-105"
                                                        onClick={() => handleChildClick(tree.token, child)} >
